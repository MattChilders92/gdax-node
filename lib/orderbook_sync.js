...
class OrderbookSync extends WebsocketClient {
  constructor(
    ...
    super(productIDs, websocketURI, auth, {channels: channels});
    this.apiURI = apiURI;
  ...
  changeGrouping(delta, productId) {
  ...
  processMessage(data) {
    ...
    if (data.sequence !== this._sequences[product_id] + 1 || this._queues.length > 50) {
